  ...
  function download(blobUrl, filename) {
    ...
    if (a.click) {
      ...
      if (window.top === window &&
          blobUrl.split('#')[0] === decodeURI(window.location.href).split('#')[0]) {
...
function validateFileURL(file) {
  try {
    var viewerOrigin = new URL(decodeURI(window.location.href)).origin || 'null';
    if (HOSTED_VIEWER_ORIGINS.indexOf(viewerOrigin) >= 0) {
      // Hosted or local viewer, allow for any file locations
      return;
    }
    var fileOrigin = new URL(file, decodeURI(window.location.href)).origin;
...
function webViewerInitialized() {
  var queryString = decodeURI(document.location.search.substring(1));
